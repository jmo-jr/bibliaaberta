...
json_file_path = 'NtIntWords.json'
...
for td in soup.find_all("table", class_="tablefloatleft"):
    # Busca pela palavra grega
    greek_span = td.find("span", class_="greek")

    if greek_span:
        greek_word = greek_span.text
    
        # Verifica se a chave já existe no dicionário
        if greek_word not in jsonNT:
            # Busca o número strong
            ...
            traducao = None
            trad_span = td.find("span", class_="eng")
            if trad_span:
              traducao = trad_span.text
